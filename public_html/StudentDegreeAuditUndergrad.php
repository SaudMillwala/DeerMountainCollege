                    ...
                    <table style="padding-bottom:10px;width:120%;">
					    <tbody style="text-align: center; ">
						    <tr colspan="5">
							    ...
							    <td>
								    <button onClick="window.location.reload(true)" onclick="window.location.href = 'StudentDegreeAuditUndergrad.php'" class=adminNav; style="background-color:#006a4e;font-size:80%;margin-top:0px;height:58px;">Degree<br>Audit</button>
							    </td>
								...
								<td><b>Current Major:</b>
								   ...
								   //Credits Total
                                        ...
                                        //Major Credits Checker
                                         ...
                                         $result = $pdo->query('SELECT SUM(course.course_Credit) as requiredsumcc FROM course, student_major,major where course.dept_ID = major.dept_ID AND major.major_ID = student_major.major_ID AND student_major.student_ID = '.$_SESSION["user_ID"]);
                                          while($row = $result->fetch(PDO::FETCH_ASSOC))
                                        { 
                                            ...
                                            {
                                             $result = $pdo->query('SELECT count(student_major.major_ID) as doublemajor FROM major, student_major WHERE major.major_ID = student_major.major_ID AND student_major.student_ID = '.$_SESSION["user_ID"]);
                                            while($row = $result->fetch(PDO::FETCH_ASSOC))
                                        { 
                                           if ('doublemajor' > 1)
                                            {
							...
							<tr>
								<td>
                                    <?php
                                        ...
                                        $result = $pdo->query('SELECT * FROM course, student_minor, minor_requirements WHERE minor_requirements.course_ID = course.course_ID AND student_minor.minor_ID = minor_requirements.minor_ID AND student_minor.student_ID = "'.$_SESSION["user_ID"].'" AND minor_requirements.course_ID NOT IN (SELECT course_ID FROM student_history WHERE student_ID = '.$_SESSION["user_ID"].')');
